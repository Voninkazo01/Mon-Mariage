...
return $toastElement;function fadeAway(){if($(':focus',$toastElement).length>0){return}
...
$toastElement.remove();$toastElement=null;if($container.children().length===0){$container.remove()}}})()})}(typeof define==='function'&&define.amd?define:function(deps,factory){if(typeof module!=='undefined'&&module.exports){module.exports=factory(require('jquery'))}else{window.toastr=factory(window.jQuery)}}));toastr.options={"debug":!1,"positionClass":"toast-bottom-right","onclick":null,"fadeIn":300,"fadeOut":100,"timeOut":5000,"extendedTimeOut":1000}
function wc_alert(data){if(data!==null&&data!==''){if(data.notice=='0'){toastr.error(data.message)}
if(data.notice=='1'){toastr.success(data.message)}
if(data.notice=='2'){toastr.info(data.message)}
if(data.notice=='3'){toastr.warning(data.message)}}}